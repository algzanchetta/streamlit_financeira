...
def load_data():
    ...
    movimentos = pd.read_sql_query("SELECT * FROM contratos_movimentacoes", conn)
    conn.close()

    # Excluir movimentacoes soft-deletadas (flag del=1)
...
def filter_cancelled_contracts(contratos, movimentos):
    """Remove contratos em status terminal (Perdido/Quitado) sem nenhum movimento (cancelados sem operacao).

    Status diferenciais nesta base:
      - Aberto: contrato ativo, em andamento
    ...
    Contratos Perdidos ou Quitados sem movimento sao considerados ruido (criados e descartados).
    """
    ...
    contratos["cancelado_sem_movimento"] = (
        contratos["status"].isin(["Perdido", "Quitado"]) & (contratos["mov_count"] == 0)
    )
...
def main():
    ...
    st.caption(
        "Carteira atual (contratos/movimentacoes): microcredito com vencimentos dinamicos - a cada pagamento "
        "efetuado gera nova data de vencimento (sistema de rolanamento). "
        "Status diferenciados: Aberto, Parcelado, Quitado, Perdido. Fonte: dbase.db."
    )
    ...
    with tab_geral:
        ...
        with st.expander("Ver análise completa", expanded=False):
            ...
            **Panorama da operação vs mercado (microcrédito brasileiro):**
            ...
            | Prazo | Vencimentos dinâmicos (rolagem diária) | 30-90 parcelas fixas | ✅ Padrão |
    ...
    with tab_controle:
        ...
        status_desc = pd.DataFrame([
            {"Status": "Aberto", "Descricao": "Contrato ativo, em andamento"},
