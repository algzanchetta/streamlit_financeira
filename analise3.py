...
def load_data():
    ...
    # --- Combinar as duas fontes de contratos ---
    # Fonte A: modelo dinamico (vencimentos rolling) - contratos + contratos_movimentacoes
    # Fonte B: modelo fixo 90x diario - contratos2 + contratos_movimentacoes2
    # As tabelas compartilham IDs sobrepostos mas com dados divergentes (snapshots diferentes)
    ct_a = pd.read_sql_query("SELECT * FROM contratos", conn)
    ct_b = pd.read_sql_query("SELECT * FROM contratos2", conn)
    ct_a["fonte"] = "contratos"
    ct_b["fonte"] = "contratos2"

    # Unificar colunas (contratos tem visitado/modelo, contratos2 tem perdido)
    for col in ("visitado", "modelo"):
        ct_b[col] = ct_b.get(col, None)
    for col in ("perdido",):
        ct_a[col] = ct_a.get(col, None)

    # Concatenar e deduplicar por ID: prefere a fonte original (contratos) para IDs sobrepostos
    ct_concat = pd.concat([ct_a, ct_b], ignore_index=True)
    ct_concat = ct_concat.sort_values("fonte", ascending=False).drop_duplicates(subset=["id"], keep="first")

    # Normalizar status: "Vigente" -> "Aberto" (ambos = ativo)
    ct_concat["status"] = ct_concat["status"].replace({"Vigente": "Aberto"})
    contratos = ct_concat.copy()

    # --- Combinar as duas fontes de movimentos ---
    mv_a = pd.read_sql_query("SELECT * FROM contratos_movimentacoes", conn)
    mv_b = pd.read_sql_query("SELECT * FROM contratos_movimentacoes2", conn)
    conn.close()

    mv_a["fonte"] = "contratos_movimentacoes"
    mv_b["fonte"] = "contratos_movimentacoes2"

    # Unificar colunas: mov_a tem abatimento/acrescimo/diasemaberto/pgtoemdia/diasareceber
    # mov_b tem tipo_desconto (e nao tem as anteriores)
    all_mv_cols = sorted(set(mv_a.columns) | set(mv_b.columns))
    for col in all_mv_cols:
        if col not in mv_a.columns:
            mv_a[col] = None
        if col not in mv_b.columns:
            mv_b[col] = None

    mv_concat = pd.concat([mv_a, mv_b], ignore_index=True)
    # Deduplicar por ID: prefere movimentos2 (fonte B) que tem dados de pagamento mais completos
    mv_concat = mv_concat.sort_values("fonte", ascending=False).drop_duplicates(subset=["id"], keep="first")
    movimentos = mv_concat.copy()

    # Excluir movimentos soft-deletados (flag del=1)
    if "del" in movimentos.columns:
        movimentos = movimentos[movimentos["del"].fillna(0) != 1].copy()
...
def filter_cancelled_contracts(contratos, movimentos):
    """Remove contratos em status terminal sem nenhum movimento (cancelados sem operacao).

    Status combinados de ambas as fontes:
      - Aberto: contrato ativo (dinamico/Vigente)
      - Parcelado: contrato ativo em regime de parcelamento
      - Quitado: contrato totalmente pago (mantido para analise de performance)
      - Perdido: contrato considerado perdido/inadimplente total
      - Finalizado: contrato encerrado (fonte B - modelo fixo)
    Contratos Perdidos, Quitados ou Finalizados sem movimento sao considerados ruido.
    """
    ...
    contratos["cancelado_sem_movimento"] = (
        contratos["status"].isin(["Perdido", "Quitado", "Finalizado"]) & (contratos["mov_count"] == 0)
    )
...
def main():
    ...
    fonte_choices = ["Todas as fontes"]
    if "fonte" in contratos.columns:
        fonte_choices += sorted(contratos["fonte"].dropna().unique().tolist())
    selected_fonte = st.sidebar.selectbox("Fonte de dados", fonte_choices, help="Filtra por modelo de contrato")
    if selected_fonte != "Todas as fontes":
        contratos = contratos[contratos["fonte"] == selected_fonte].copy()
        # Mapear fonte de contratos para fonte de movimentos
        mov_fonte_map = {
            "contratos": "contratos_movimentacoes",
            "contratos2": "contratos_movimentacoes2",
        }
        mv_fonte = mov_fonte_map.get(selected_fonte, selected_fonte)
        if "fonte" in movimentos.columns:
            movimentos = movimentos[movimentos["fonte"] == mv_fonte].copy()
    ...
    st.caption(
        "Analise combinada de ambas as fontes: contratos/contratos_movimentacoes (modelo dinamico - "
        "vencimentos rolling, rolanamento) + contratos2/contratos_movimentacoes2 (modelo fixo 90x diario Pix). "
        "Status diferenciados: Aberto, Parcelado, Quitado, Perdido, Finalizado. Fonte: dbase.db."
    )
    ...
    with tab_geral:
        ...
        with st.expander("Ver análise completa", expanded=False):
            ...
            **Panorama da operação vs mercado (microcrédito brasileiro):**
            ...
            | Prazo | Dinâmico (rolagem) + Fixo 90x | 30-90 parcelas | ✅ Ambos padrão |
    ...
    with tab_controle:
        ...
        with st.expander("Como ler esta aba", expanded=False):
            ...
            **Esta aba mostra o controle administrativo dos contratos.**
            ...
            - **Resumo por situação**: total de contratos por status (Aberto, Parcelado, Quitado, Perdido)
            ...
            - **Portfólio total**: contratos válidos (após exclusões)
            """)
        ...
        status_desc = pd.DataFrame([
            {"Status": "Aberto", "Descricao": "Contrato ativo (modelo dinamico ou Vigente)"},
            {"Status": "Parcelado", "Descricao": "Contrato ativo em regime de parcelamento"},
            {"Status": "Quitado", "Descricao": "Contrato totalmente pago (performer)"},
            {"Status": "Perdido", "Descricao": "Contrato considerado perdido/inadimplente total"},
            {"Status": "Finalizado", "Descricao": "Contrato encerrado (fonte modelo fixo 90x)"},
        ])
        st.markdown("**Legenda de status diferenciados:**")
        st.dataframe(status_desc, hide_index=True, use_container_width=True)
